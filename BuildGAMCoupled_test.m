...
Y = double(Yt); % (bins with >1 spike counted as 1 spike ---> if this matters, use small enough bins)
Ytype = {'event'}; % spikes are events
binrange_y = round([.006 ; 0.246]/dt);
nbins_y = 5;
basistype_y = 'nlraisedcosine';
nprsY = {nbins_y*size(Y,2)};
for i=1:size(Y,2)
    [y{i},basis_y{i},nprs_y{i}] = RecodeInput(Y(:,i), Ytype, binrange_y, nbins_y, basistype_y, dt);
end
%% combine input variables with spikes from other neurons
X = [cell2mat(x) cell2mat(y)];
x = [x y];
Px = [Px cellfun(@(z) sum(z~=0)/size(z,1),y,'UniformOutput',false)];
Xtype = [xtype Ytype];
nprs = cell2mat([nprs nprsY]);
Lambda = [lambda lambdaY];
nvars = nvars + 1;
basis{end+1}.x = cell2mat(cellfun(@(z) z.x, basis_y, 'UniformOutput', false)); 
basis{end}.y = cell2mat(cellfun(@(z) z.y, basis_y, 'UniformOutput', false));
